...
<table class="table table-success table-striped">
    ...
    <c:forEach var="product" items="${productList}" >
        <tr>
            ...
            <td>
                <c:choose>
                    <c:when test="${product.categoryId == 1 }">Điện thoại</c:when>
                    <c:when test="${product.categoryId == 2 }">Tivi</c:when>
                    <c:when test="${product.categoryId == 3 }">Tủ lạnh</c:when>
                    <c:when test="${product.categoryId == 4 }">Máy giặt</c:when>
                    <c:when test="${product.categoryId == 5 }">Máy tính</c:when>
                </c:choose>
            </td>
            <td>

                <button class="btn btn-warning" onclick="window.location.href='/product?action=edit&id=${product.id}'">Edit</button>

                <!-- Button trigger modal -->
                <button type="button" onclick="showInfoDelete('${product.id}','${product.name}')"
            ...
            <div class="modal-content">
                <form action="/product?action=delete" method="post">
                    ...
                    <div class="modal-body">
                        ...
                        <span>Bạn có muốn xóa mặt hàng : </span><span id="nameDelete" style="color: red"></span>
                    </div>
